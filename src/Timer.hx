...
class Timer {
	...
	static function update(delta:Float, realDelta:Float):Void {
		...
		for (i in 0...callbacks.length) {
			var callback = callbacks.unsafeGet(i);
			if (callback.cleared) {
				callback.time += delta;
			}
		}
	...
	/**
				 ...
				 * ```
				 */
	inline public static function interval(#if ceramic_optional_owner ?owner:Entity #else owner:Null<Entity> #end, seconds:Float,
